...
addRandomScript("سكربت بحث سكربتات من موقع ScriptSearcher",
    "https://raw.githubusercontent.com/AZYsGithub/chillz-workshop/main/ScriptSearcher"
)
addRandomScript("سكربت الانتقال الفوري إلى اللاعبين",
...
addRandomScript("سكربت SaveInstance (ينسخ اللعبة كاملة)",
    "https://raw.githubusercontent.com/24rr/tmpignore/refs/heads/main/32423sadf133"
)
addRandomScript("سكربت أدوات فينوكس ويب هوك",
...
addHackCopy("هاك Delta",
    "https://raw.githubusercontent.com/ElijahGamingRBLX/Roblox-Executor/8536d27eef4eca985572dc30fc146dfd4e55a893/DeltaExecutor.lua"
)
...
BehaviorTab:CreateSlider({
    Name         = "قوة القفز",
    Range        = {50, 300}, -- من 50 إلى 100
    Increment    = 1,         -- كل حركة تزيد/تنقص 1
    CurrentValue = 50,        -- القيمة الابتدائية
    Callback     = function(value)
        local char = Players.LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
...
BehaviorTab:CreateToggle({
    ...
    Callback = function(isNight)
        ...
        if isNight then
            -- 🌙 الوضع الليلي
            Lighting.ClockTime        = 20 -- تعني الساعة 8 مساءً
            Lighting.Brightness       = 1
            Lighting.FogEnd           = 100
            Lighting.OutdoorAmbient   = Color3.fromRGB(50, 50, 70)
            Lighting.Ambient          = Color3.fromRGB(30, 30, 50)
        else
            -- 🌞 الوضع الصباحي (يتم إصلاح الشاشة الرمادية هنا)
            Lighting.ClockTime        = 8 -- تعني الساعة 8 صباحًا
...
BehaviorTab:CreateLabel({
	Text = "يتم اضافة اشياء بعد انشالله",
	Style = 1
})
